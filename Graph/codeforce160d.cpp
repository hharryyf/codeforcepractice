...
int main() {
    ...
    for (i = 1; i <= m; ++i) {
        if (edg[i].select) {
            if (depth[edg[i].from] > depth[edg[i].to]) {
                swap(edg[i].from, edg[i].to); 
            } 

            wt[edg[i].to] = edg[i].w;
            ans[edg[i].id] = 1;
        } else {
            change(edg[i].from, edg[i].to, edg[i].w);
        }   
    }
    ...
    for (i = 1; i <= m; ++i) {
        if (edg[i].select) {
            if (t2.query(id[edg[i].to], id[edg[i].to], 1, n, 1) > edg[i].w) {
                ans[edg[i].id]++;
            }
        } else if (hld(edg[i].from, edg[i].to) == edg[i].w) {
            ans[edg[i].id] = 1;
        }
    }
